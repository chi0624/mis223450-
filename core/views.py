        ...
        if form.is_valid():
            ...
            if role == 'teacher':
                if teacher_code != TEACHER_CODE:
                    messages.error(request, '❌ 教師驗證碼錯誤，請重新輸入')
                    user.delete()
            ...
            # 登入並導向
            login(request, user)
            messages.success(request, f"✅ { '老師' if role == 'teacher' else '學生' } 註冊成功！")
            return redirect('dashboard')

        else:
            messages.error(request, '❌ 註冊資料有誤，請重新輸入。')
...
def lecture_submissions(request, lecture_id):
    ...
    # 對每個學生統計該講次的作答情況
    students_data = []
    for student in Student.objects.select_related('user','user__profile').filter(user__profile__role='student'):
...
def student_directory(request):
    ...
    students = (
        ...
        .filter(user__profile__role='student')
